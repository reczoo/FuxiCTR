...
class DIEN(BaseModel):
    ...
    Args:
        ...
        dnn_hidden_units (list): Hidden units of the DNN. Default: ``[200, 80]``.
        ...
        embedding_dim (int): Embedding dimension of features. Default: ``16``.
        net_dropout (float): Dropout rate for DNN. Default: ``0``.
        batch_norm (bool): Whether to apply batch normalization. Default: ``True``.
        gru_type (str): GRU variant, one of ``"GRU"``, ``"AIGRU"``, ``"AGRU"``, or ``"AUGRU"``.
            Default: ``"AUGRU"``.
        enable_sum_pooling (bool): Whether to enable sum pooling of behavior sequence.
            Default: ``False``.
        attention_dropout (float): Dropout rate for attention layers. Default: ``0``.
        attention_type (str): Attention mechanism type. Default: ``"bilinear_attention"``.
        attention_hidden_units (list): Hidden units of attention MLP. Default: ``[80, 40]``.
        attention_activation (str): Activation for attention MLP. Default: ``"Dice"``.
        use_attention_softmax (bool): Whether to apply softmax on attention scores.
            Default: ``True``.
        item_info_fields (int): Number of item info fields. Default: ``1``.
    ...
    def get_unmasked_tensor(self, h, non_zero_mask):
        """Restore masked-out rows into a full batch tensor.

        Args:
            h: Tensor of shape ``(num_nonzero, ...)``.
            non_zero_mask: Boolean mask indicating non-zero rows.

        Returns:
            Tensor: Full batch tensor with zeros for masked rows.
        """
    ...
    def interest_extraction(self, idx, sequence_emb, mask):
        """Extract user interest from behavior sequence using GRU.

        Args:
            idx: Index of the extraction module to use.
    ...
    def interest_evolution(self, idx, packed_interests, interest_emb, target_emb, mask):
        """Evolve user interest with attention-augmented GRU.

        Args:
            idx: Index of the evolution module to use.
            packed_interests: Packed sequence of interests.
            interest_emb: Padded interest embeddings.
            target_emb: Target item embedding.
            mask: Sequence padding mask.

        Returns:
            Tensor: Evolved interest representation.
        """
...
class AttentionLayer(nn.Module):
    """Attention layer for DIEN with multiple attention mechanisms.

    Args:
        model_dim (int): Dimension of the model embeddings.
        attention_type (str): Type of attention, one of ``"bilinear_attention"``,
            ``"dot_attention"``, or ``"din_attention"``. Default: ``"bilinear_attention"``.
        attention_hidden_units (list): Hidden units for DIN-style attention MLP.
            Default: ``[80, 40]``.
        attention_activation (str): Activation function for attention MLP. Default: ``"Dice"``.
        use_attention_softmax (bool): Whether to apply softmax on attention scores.
            Default: ``True``.
        attention_dropout (float): Dropout rate for attention MLP. Default: ``0.0``.
    """
    ...
    def forward(self, sequence_emb, target_emb, mask=None):
        """Compute attention scores between sequence and target embeddings.

        Args:
            sequence_emb: Sequence embeddings of shape ``(batch_size, seq_len, dim)``.
            target_emb: Target embedding of shape ``(batch_size, dim)``.
            mask: Optional padding mask.

        Returns:
            Tensor: Attention scores of shape ``(batch_size, seq_len)``.
        """
...
class AGRUCell(nn.Module):
    ...
    def forward(self, x, hx, attn):
        ...
        Args:
            ...
            hx: Previous hidden state of shape ``(batch, hidden_size)``.
            attn: Attention scores of shape ``(batch,)``.

        Returns:
            Tensor: Updated hidden state of shape ``(batch, hidden_size)``.
        """
        gate_x = self.x2h(x)
        gate_h = self.h2h(hx)
...
class DynamicGRU(nn.Module):
    ...
    Args:
        ...
        hidden_size (int): Hidden state dimension.
        bias (bool): Whether to use bias in linear layers. Default: ``True``.
        gru_type (str): GRU variant, one of ``"AUGRU"`` or ``"AGRU"``. Default: ``"AUGRU"``.
    """
    ...
    def forward(self, packed_seq_emb, attn_score=None, h=None):
        ...
        Returns:
            tuple: ``(packed_outputs, hidden_state)``.
        """
