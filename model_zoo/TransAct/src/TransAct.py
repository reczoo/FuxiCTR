...
class TransAct(BaseModel):
    ...
    Args:
        ...
        target_item_field (list): which field is used for target item embedding.
            When tuple is applied, the fields in each tuple are concatenated. Default: ``[("item_id", "cate_id")]``.
        sequence_item_field (list): which field is used for sequence item embedding.
            When tuple is applied, the fields in each tuple are concatenated. Default: ``[("click_history", "cate_history")]``.
    ...
    def concat_embedding(self, field, feature_emb_dict):
        """Concatenate embeddings for a given field tuple or string.
        ...
        Returns:
            Tensor: Concatenated embedding tensor.
        """
        if type(field) == tuple:
